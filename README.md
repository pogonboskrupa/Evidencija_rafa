...
- **Registracija i prijava s PIN-om** putem prave numeričke tipkovnice (dodirom ili fizičkom tipkovnicom) — bez tekstualnog polja za lozinku. PIN ima fiksno 4 cifre; radnja se pokreće automatski čim je upisana zadnja cifra, bez posebnog dugmeta za potvrdu. Registracija ide u dva koraka (postavi pa potvrdi PIN), s mogućnošću povratka na prvi korak. Uz to, opcija "Zapamti me" da se ne morate prijavljivati svaki put.
  - Tastatura se ne prikazuje odmah — pojavljuje se tek klikom/dodirom na polje za PIN (ili prvim pritiskom cifre na fizičkoj tipkovnici), da ne zauzima prostor na ekranu prije nego što je stvarno potrebna.
...
- **Kartica "Raspored pločica"** — raspored markirnih pločica po odjelima:
  ...
  - Aplikacija **pamti imena ranije unesenih radnika** (npr. iste ekipe od 7-8 ljudi koja se raspoređuje po više odjela) i nudi ih kao prijedlog pri kucanju imena u bilo kojem odjelu — ime ostaje zapamćeno trajno, i nakon brisanja tog unosa.
...
- **Godišnji pregled** — cijela godina (12 mjeseci) prikazana na jednom listu, sa statistikom (ukupno radnih dana, ukupan broj stabala, površina, kilometraža vlaka, iskorišteni/preostali godišnji odmor...) i mogućnošću ispisa/PDF-a.
  - **Raspored pločica po radniku** — zbirna tabela na dnu Godišnjeg pregleda: koliko je pločica/paketa ukupno zaduženo svakom radniku, sabrano kroz sve odjele u "Rasporedu pločica" (i broj odjela u kojima se pojavljuje), umjesto da se to ručno sabira po odjelima.
- **Tamna tema** (Postavke → Izgled) — prekidač "Prati sistem / Svijetla / Tamna". Podrazumijevano prati postavku uređaja/preglednika; ručni izbor se pamti po uređaju (ne po korisničkom računu) i vrijedi i na ekranu za prijavu, bez bljeska pogrešne teme pri učitavanju. Upozorenja (praznina u rasporedu pločica, zona brisanja računa) imaju vlastite tamne varijante, umjesto svijetlih blokova sa nečitljivim tekstom.
- **Status zadataka na prvi pogled** — oznaka uz dan je žuta dok nešto stoji nezavršeno, a zelena kad su svi zadaci tog dana završeni (isti dogovor o bojama kao tačkice u godišnjem kalendaru).
